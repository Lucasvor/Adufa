...
- 调整每个应用当前的音量和静音状态。
- 在受支持的 Windows 11 版本中，右键单击正在发声的任务栏应用，可在原生菜单旁
  打开实验性的输出、音量和静音控制。
...
在受支持的 Windows 11 版本中，右键单击正在发声应用的任务栏图标，可以在原生
任务栏菜单旁打开 Adufa 的紧凑辅助面板。原生菜单仍然可用；Adufa 只是补充音量和
输出控制。

此集成需要将可见任务栏图标与活动音频应用进行匹配。它仍是测试版功能；当 Windows
无法提供可靠匹配时，可以改用全局快捷键或托盘弹窗。
...
1. 让目标应用至少播放一次音频，以便 Windows 暴露音频会话。
2. 右键单击其任务栏图标。
3. 在相邻的 Adufa 面板中静音、调整音量或选择输出。
4. 选择输出后，辅助面板和原生菜单都会关闭。

如果辅助面板没有出现，请在指向正在发声的应用时按 `Ctrl + Alt + A`，或从通知区域打开 Adufa。
...
| 右键单击任务栏中正在发声的应用 | 在原生菜单旁打开实验性 Adufa 辅助面板 |
| `Tab` 或 `↓` | 移至下一项 |
| `↑` | 移至上一项 |
